...
2. 모호한 점을 `AskUserQuestion`으로 **한 번에 묶어** 확인한다. 확인할 카테고리:
   ...
   - 수용 기준 (성공을 어떻게 판단하나) — QA가 빌드 통과가 아니라 **실제 동작/데이터**로 대조할 수 있는 형태로.
   - **도메인 상태 확정** (요청에 "X 상태에서…" 가 있으면): X 가 단일 enum 값인지 집합(`HOLDING_CAR_STATUSES` 류)인지, BEFORE/AFTER(인수 직전/직후) 중 무엇인지를 추측 말고 코드 grep + 사용자 한 줄 컨펌으로 확정 → spec 에 enum 값으로 명시. (단어 추측이 lessons.md 정정의 단골 원인.)
